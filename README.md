...
This can be annoying, so we can it a bit easier for ourselves by registering the dataset.
Registering a dataset is just a matter of telling AzureML where the data is stored and giving it a name.
Then in the future, we can just download the data by using `Model(name=name, version=version, workspace=ws).download(path)`.
...
script_run_config = ScriptRunConfig(
    ...
    compute_target=compute_target_name, # The name of the compute target can be found in the AzureML under Compute -> Compute clusters
...
This scripts run the command the following script in sequances on the compute cluster:
...
We can do this by using the `Model` class ([docs](https://docs.microsoft.com/en-us/python/api/azureml-core/azureml.core.model.model?view=azure-ml-py)).
...
You can also try to download the model using:
...
Second, you can build it locally and push it to your private Azure Container Registry.
```bash
az login
...
Another small thing we need to do is to change the bash command we want to run since we are now using Poetry.
You can see the full code in the `azureml_scripts/submit_job_custom_docker_image.py` script.
Have a look at the code and make sure you understand what is happening.
When you are done, you can run the script using:
